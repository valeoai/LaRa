...
class LaRaEncoder(nn.Module):
    def __init__(
        ...
        Args:
            ...
            layerscale_init: intial value for LayerScale in self-attention layers
            drop_path: drop rate for stochastic depth on self-attention layers
        ...
        self.sa_layer = self_attention_block(
            num_layers=num_self_attention_layers_per_block,
            num_channels=num_latent_channels,
            num_heads=num_self_attention_heads,
            scale_init=layerscale_init,
            drop_path=drop_path,
            activation_checkpoint=activation_checkpoint,
        )
    ...
    def forward(self, x, rots, trans, intrins, input_stride, pad_mask=None):
        ...
        x_latent = self.ca_layer(x_latent, x, pad_mask)

        for i in range(self.num_layers):
            x_latent = self.sa_layer(x_latent, pad_mask)
