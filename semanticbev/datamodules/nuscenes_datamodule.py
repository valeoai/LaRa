...
class VisualThenMotionNuScenesDataModule(pl.LightningDataModule):
    """
    LightningDataModule for the Nuscenes dataset.
    ...
    def __init__(self, version, dataroot, data_aug_conf, grid_conf, batch_size, num_workers, pin_memory,
                 train_shuffle=True, prefetch_factor=4, update_intrinsics=True):
        super().__init__()
    ...
    def setup(self, stage):
        ...
        self.motion_trainloader, self.motion_valloader = compile_data(self.version,
                                                        self.dataroot,
                                                        data_aug_conf=self.data_aug_conf,
                                                        grid_conf=self.grid_conf,
                                                        bsz=self.batch_size,
                                                        nworkers=self.num_workers,
                                                        pin_memory=self.pin_memory,
                                                        parser_name='egomotion_segmentationdata',
                                                        train_shuffle=self.train_shuffle,
                                                        prefetch_factor=self.prefetch_factor,
                                                        update_intrinsics=self.update_intrinsics
                                                        )
    ...
    def train_dataloader(self):
        ...
        return [self.trainloader, self.motion_trainloader]

    def val_dataloader(self):
        return [self.valloader, self.motion_valloader]